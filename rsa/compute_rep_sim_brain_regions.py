  ...
  for encoder_type in encoder_types:
    for embedding_type in layers:
      ...
      for context_size in np.arange(7):
        ...
        a = pickle.load(
          open(os.path.join(hparams.emb_save_dir,
                            encoder_type + '_' + 'none' + '_sentence_' + str(
                              context_size) + '-0_onlypast-True_' + embedding_type),
               'rb'))
        time_steps = a['time_steps']
        selected_steps = {1: {}, 2: {}, 3: {}, 4: {}}
        for b in blocks:
          ...
          print(a['start_steps'][b], a['end_steps'][b])
          embeddings[embedding_key][-1].extend(
            list(np.asarray(a['encoded_stimuli'][b])[selected_steps[b][embedding_key + "_" + str(context_size)]]))
  ...
  for context_size in [1]:
    ...
    for bi in np.arange(len(brain_regions_labels)):
      ...
      for key in embeddings:
        ...
        x, C = get_dists([brain_regions[bi]] + [embeddings[key][context_size]])
        klz, prz, labels_ = compute_dist_of_dists(x, C, brain_regions_labels[bi] + [embedding_key])
