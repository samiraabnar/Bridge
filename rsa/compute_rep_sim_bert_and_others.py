...
if __name__ == '__main__':
  ...
  for encoder_type in encoder_types:
    for embedding_type in layers:
      ...
      bert_keys.append(embedding_key)
  ...
  for encoder_type in encoder_types:
    for embedding_type in embedding_types[encoder_type]:
      ...
      for context_size in [0,1,6]:
        ...
        a = pickle.load(
          open(os.path.join(hparams.emb_save_dir,
                            encoder_type + '_' + embedding_type + '_sentence_' + str(
                              context_size) + '-0_onlypast-True'),
               'rb'))
        time_steps = a['time_steps']
        selected_steps = {1: {}, 2: {}, 3: {}, 4: {}}
        for b in blocks:
  ...
  import csv
  with open('bert_model_sim_prz.csv', 'w') as f:
    writer = csv.writer(f)
