...
tf.flags.DEFINE_string('embedding_type', 'lstm_0', 'ELMO: word_emb, lstm_outputs1, lstm_outputs2, elmo ')
...
if __name__ == '__main__':
  ...
  b = np.load('../bridge_data/processed/harrypotter/1_none_window' + str(0) + '-0_stimuli_in_context.npy').item()
  a = pickle.load(
    open('../bridge_models/embeddings/google_lm_' + hparams.embedding_type + '_none_' + str(0) + '-0_onlypast-True',
         'rb'))
  ...
  pickle.dump(saving_dic, open(
    '../bridge_models/embeddings/google_lm_' + hparams.embedding_type + '_none_' + str(0) + '-0_onlypast-True',
    'wb'))
